...
import {
	Paired12,
	YlGnBu3,
} from "chartjs-plugin-colorschemes/src/colorschemes/colorschemes.brewer";
...
export function createChartEstados(
	totalRecuperados,
	totalFallecidos,
	totalCasa,
	totalHospital,
...
export function createChartRangoEdad(
	totalRangoCasa,
	totalRangoRecuperado,
	totalRangoFallecido,
	totalRangoHospital,
	totalRangoUci
) {
	...
	const dataCasa10 = totalRangoCasa.filter(
		(entry) => entry.edad >= 0 && entry.edad <= 10
	);

	const dataRecuperado10 = totalRangoRecuperado.filter(
		(entry) => entry.edad >= 0 && entry.edad <= 10
	);

	const dataFallecido10 = totalRangoFallecido.filter(
		(entry) => entry.edad >= 0 && entry.edad <= 10
	);

	const dataHospital10 = totalRangoHospital.filter(
		(entry) => entry.edad >= 0 && entry.edad <= 10
	);

	const dataUci10 = totalRangoUci.filter(
		(entry) => entry.edad >= 0 && entry.edad <= 10
	);
	...
	const sumTotalRecuperado10 =
		dataRecuperado10.length > 0 ? sumTotal(dataRecuperado10) : 0;
	const sumTotalFallecido10 =
		dataFallecido10.length > 0 ? sumTotal(dataFallecido10) : 0;
	const sumTotalHospital10 =
		dataHospital10.length > 0 ? sumTotal(dataHospital10) : 0;
	const sumTotalUci10 = dataUci10.length > 0 ? sumTotal(dataUci10) : 0;

	console.log(
		sumTotalCasa10,
		sumTotalRecuperado10,
		sumTotalFallecido10,
		sumTotalHospital10,
		sumTotalUci10
	);
